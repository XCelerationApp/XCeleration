...
/// Applies the v17 migration SQL (mirrors _onUpgrade in DatabaseConnectionProvider).
Future<void> _applyV17Migration(Database db) async {
  try {
    await db
        .execute('ALTER TABLE race_participants ADD COLUMN race_uuid TEXT');
  } catch (_) {}
  try {
    await db
        .execute('ALTER TABLE race_participants ADD COLUMN runner_uuid TEXT');
  } catch (_) {}
  try {
    await db
        .execute('ALTER TABLE race_participants ADD COLUMN team_uuid TEXT');
  } catch (_) {}
}
...
void main() {
  ...
  group('DatabaseConnectionProvider v17 migration', () {
    ...
    test('adds race_uuid column to stale race_participants table', () async {
      await _applyV17Migration(db);

      await db.execute(
        "INSERT INTO race_participants (race_id, runner_id, team_id, race_uuid, is_dirty) VALUES (1, 1, 1, 'race-uuid-1', 0)",
      );
    ...
    test('adds runner_uuid column to stale race_participants table', () async {
      await _applyV17Migration(db);

      await db.execute(
        "INSERT INTO race_participants (race_id, runner_id, team_id, runner_uuid, is_dirty) VALUES (1, 1, 1, 'runner-uuid-1', 0)",
      );
    ...
    test('adds team_uuid column to stale race_participants table', () async {
      await _applyV17Migration(db);

      await db.execute(
        "INSERT INTO race_participants (race_id, runner_id, team_id, team_uuid, is_dirty) VALUES (1, 1, 1, 'team-uuid-1', 0)",
      );

      final rows = await db.query('race_participants');
      expect(rows.first['team_uuid'], equals('team-uuid-1'));
    });
    ...
    test('UPDATE race_participants SET race_uuid succeeds after migration', () async {
      ...
      await db.execute(
          'INSERT INTO race_participants (race_id, runner_id, team_id, is_dirty) VALUES (1, 1, 1, 0)');
    ...
    test('migration is idempotent on a DB that already has the columns', () async {
      await _applyV17Migration(db);
      await expectLater(_applyV17Migration(db), completes);
    });
  });
}
