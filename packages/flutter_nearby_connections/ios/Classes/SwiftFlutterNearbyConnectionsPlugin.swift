...
public class SwiftFlutterNearbyConnectionsPlugin: NSObject, FlutterPlugin {
    ...
    // MultipeerConnectivity reports on its own background queues, and a
    // platform channel may only be used from the main thread: off it, Flutter
    // can drop the message or crash. XCeleration change, not in the upstream
    // plugin.
    @objc func stateChanged(){
        ...
        let json = JSON(devices.compactMap({return $0.toStringAnyObject()})).rawString()
        DispatchQueue.main.async {
            self.channel.invokeMethod(INVOKE_CHANGE_STATE_METHOD, arguments: json)
        }
    }
